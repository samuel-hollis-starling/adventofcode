...
public class FirstQuestion {
    ...
    public static void main(String[] args) {
        ...
        for (int i = split + 1; i < list.size(); i++) {
        ...
        for (String page : pages) {
            ...
            List<Integer> pageNumbers = Arrays.stream(page.split(",")).map(Integer::parseInt).toList();
            for (String rule : rules) {
                followsRules = followsRules && followsRule(rule, pageNumbers);
                if (!followsRules) {
                    break;
                }
            }
            if (followsRules) {
                sum += pageNumbers.get(pageNumbers.size() / 2 + 1);
            }
        }
        System.out.println(sum);
    }

    public static boolean followsRule(String rule, List<Integer> pageNumbers) {
        String[] beforeAndAfter = rule.split("\\|");
        int before = Integer.parseInt(beforeAndAfter[0]);
        int after = Integer.parseInt(beforeAndAfter[1]);
        int indexBefore = pageNumbers.indexOf(before);
        int indexAfter = pageNumbers.indexOf(after);
        if (indexBefore == -1 || indexAfter == -1) {
            return true;
        }
        else return indexBefore <= indexAfter;
    }
